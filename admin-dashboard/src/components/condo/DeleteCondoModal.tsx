...
export const DeleteCondoModal = ({
  ...
  const handleDelete = async () => {
    try {
      ...
      await deleteCondoApi(condo.id)
      onSuccess()
      onClose()
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to delete condominium.'
      setErrorMessage(msg)
    } finally {
      setIsDeleting(false)
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl border border-gray-800 bg-[#0F131C] p-6 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Confirm deletion for {condo.condoCode}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="mt-4 text-xs text-gray-300">
          Are you sure you want to delete <span className="font-bold text-gray-100">{condo.condoName}</span>? This action cannot be undone.
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-3 border-t border-gray-800 pt-4">
          ...
          >
            {isDeleting ? 'Deleting...' : 'Delete Condominium'}
          </button>
        </div>
      </div>
    </div>
...
export default DeleteCondoModal
